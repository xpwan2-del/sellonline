...
const { appVersion, adminBrandName, siteURL, loadPublicConfig } = useAdminBrand()
const adminControlRoom = computed(() => t('admin.layout.controlRoom', { brand: adminBrandName.value }))
const adminWorkspace = computed(() => t('admin.layout.workspace', { brand: adminBrandName.value }))
const collapsedBrand = computed(() => {
  const chars = adminBrandName.value
    .replace(/\badmin\b/ig, '')
    .trim()
    .split(/\s+/)
    .filter(Boolean)
    .map((part) => part[0])
    .join('')
  return (chars || 'A').slice(0, 3).toUpperCase()
})
...
const navGroups = computed<NavGroup[]>(() => {
  const groups: NavGroup[] = [
    {
      id: 'dashboard',
      label: t('admin.navGroups.dashboard'),
      icon: LayoutDashboard,
      items: [
        {
          label: t('admin.navItems.dashboardOverview'),
          to: '/',
          icon: LayoutDashboard,
        },
        {
          label: t('admin.navItems.visitAnalytics'),
          to: '/visit-analytics',
          icon: BarChart3,
          permission: 'GET:/admin/analytics/visits/summary',
        },
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        {
          label: t('admin.navItems.affiliatesInviteCodes'),
          to: '/affiliates/invite-codes',
          icon: KeySquare,
          permission: 'GET:/admin/affiliate-invite-codes',
        },
        {
          label: t('admin.navItems.affiliatesApplications'),
          to: '/affiliates/applications',
          icon: ClipboardList,
          permission: 'GET:/admin/affiliate-applications',
        },
        {
        ...
        {
          label: t('admin.navItems.affiliatesReports'),
          to: '/affiliates/reports',
          icon: BarChart3,
          permission: 'GET:/admin/affiliates/reports/summary',
        },
        {
    ...
    <div class="flex min-h-screen">
      ...
      >
        <div class="py-6" :class="sidebarCollapsed ? 'px-2' : 'px-6'">
          ...
          <div v-if="!sidebarCollapsed" class="text-xs text-muted-foreground mt-1">{{ adminControlRoom }}</div>
          <div v-if="sidebarCollapsed" class="text-lg font-semibold tracking-tight text-center">{{ collapsedBrand }}</div>
        </div>
        ...
        <nav class="pb-3 space-y-2 flex-1 overflow-y-auto" :class="sidebarCollapsed ? 'px-1.5' : 'px-3'">
          ...
          <template v-if="!sidebarCollapsed">
            <div
              v-if="filteredNavGroups.length === 0"
        ...
        <div class="border-t border-border">
          <div v-if="!sidebarCollapsed" class="px-6 py-3 text-[11px] text-muted-foreground space-y-1">
            <p>© {{ new Date().getFullYear() }} {{ adminBrandName }} <span v-if="appVersion" class="text-muted-foreground/70">{{ appVersion }}</span></p>
            <a
              :href="displaySiteURL"
              target="_blank"
      ...
      <Sheet v-model:open="mobileNavOpen">
        <SheetContent side="left" class="w-72 p-0 flex flex-col">
          ...
          <div class="px-6 py-6">
            ...
            <div class="text-xs text-muted-foreground mt-1">{{ adminControlRoom }}</div>
          </div>
          <div class="px-3 pb-2">
            <Input
              v-model="navSearch"
      ...
      <div class="flex-1 flex flex-col min-w-0">
        <header class="flex items-center justify-between border-b border-border bg-background px-4 md:px-8 py-3 md:py-4 gap-2">
          <div class="flex items-center gap-3">
            ...
            <div class="text-sm text-muted-foreground hidden sm:block">{{ adminWorkspace }}</div>
          </div>
