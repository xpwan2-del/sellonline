...
const qqQrPickerRef = ref<InstanceType<typeof MediaPicker> | null>(null)
...
const openQQQrPicker = () => {
  qqQrPickerRef.value?.openPicker()
}

const clearQQQrImage = () => {
  form.contact.qq_qr_image = ''
}
    ...
    <Tabs v-model="currentTab" class="flex flex-col gap-6">
      ...
      <div class="rounded-xl border border-border bg-card">
        ...
        <div class="grid grid-cols-1 gap-6 p-6 md:grid-cols-3">
          <div class="space-y-2">
            <label class="text-xs font-medium text-muted-foreground">{{ t('admin.settings.contact.qqNumber') }}</label>
            <Input v-model="form.contact.qq_number" :placeholder="t('admin.settings.contact.qqNumberPlaceholder')" />
          </div>
          <div class="space-y-2">
            <label class="text-xs font-medium text-muted-foreground">{{ t('admin.settings.contact.qqGroup') }}</label>
            <Input v-model="form.contact.qq_group" :placeholder="t('admin.settings.contact.qqGroupPlaceholder')" />
          </div>
          <div class="space-y-2">
          ...
          <div class="space-y-2 md:col-span-3">
            <label class="text-xs font-medium text-muted-foreground">{{ t('admin.settings.contact.qqQrImage') }}</label>
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
              <button
                type="button"
                class="flex h-28 w-28 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-border bg-muted/20 transition-colors hover:border-primary/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                :title="t('admin.settings.contact.qqQrImageTip')"
                @click="openQQQrPicker"
              >
                <img v-if="form.contact.qq_qr_image" :src="getImageUrl(form.contact.qq_qr_image)" class="h-full w-full object-contain" alt="" />
                <span v-else class="px-2 text-center text-xs font-semibold text-muted-foreground">QR</span>
              </button>
              <div class="flex flex-wrap items-center gap-2">
                <Button type="button" variant="outline" size="sm" @click="openQQQrPicker">
                  {{ t('admin.settings.contact.qqQrImageSelect') }}
                </Button>
                <Button v-if="form.contact.qq_qr_image" type="button" variant="ghost" size="sm" @click="clearQQQrImage">
                  {{ t('admin.common.delete') }}
                </Button>
              </div>
            </div>
            <p class="text-xs text-muted-foreground">{{ t('admin.settings.contact.qqQrImageTip') }}</p>
            <MediaPicker ref="qqQrPickerRef" v-model="form.contact.qq_qr_image" scene="contact" dialog-only />
          </div>
        </div>
      </div>
