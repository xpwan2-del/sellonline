...
const form = reactive({
  ...
  price_amount: 0,
  cost_price_amount: 0,
  affiliate_commission_rate: '' as number | '',
...
const populateForm = (product: AdminProduct) => {
  ...
  Object.assign(form, {
    ...
    affiliate_commission_rate: product.affiliate_commission_rate === null || product.affiliate_commission_rate === undefined || product.affiliate_commission_rate === ''
      ? ''
      : Number(product.affiliate_commission_rate || 0),
    ...
    sold_count_offset: toSafeInt(product.sold_count_offset),
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      sold_count_offset: toSafeInt(form.sold_count_offset),
      skus: normalizedSKUs,
      ...
      affiliate_commission_rate: form.is_affiliate_enabled && form.affiliate_commission_rate !== ''
        ? Number(form.affiliate_commission_rate)
        : null,
    ...
    <DialogScrollContent class="w-[calc(100vw-1rem)] max-w-5xl p-4 sm:p-6" @interact-outside="(e) => e.preventDefault()">
      ...
      <form class="space-y-6" @submit.prevent="handleSubmit">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="col-span-1">
            <label class="block text-xs font-medium text-muted-foreground mb-1.5">{{ t('admin.products.form.soldCountOffset') }}</label>
            <Input
              v-model.number="form.sold_count_offset"
              type="number"
              min="0"
              step="1"
              :placeholder="t('admin.products.form.soldCountOffsetPlaceholder')"
            />
            <p class="mt-1 text-xs text-muted-foreground">{{ t('admin.products.form.soldCountOffsetTip') }}</p>
          </div>
          ...
          <div class="col-span-1 md:col-span-2 flex flex-col items-start gap-4 border-t border-border pt-4 sm:flex-row sm:flex-wrap sm:items-center sm:gap-6">
            ...
            <div class="w-full max-w-xs space-y-1.5">
              <label class="text-xs font-medium text-muted-foreground">{{ t('admin.products.form.affiliateCommissionRate') }}</label>
              <Input
                ...
                max="100"
                step="0.01"
                :disabled="!form.is_affiliate_enabled"
                :placeholder="t('admin.products.form.affiliateCommissionRatePlaceholder')"
              />
              <p class="text-xs text-muted-foreground">{{ t('admin.products.form.affiliateCommissionRateHint') }}</p>
            </div>
            <div class="inline-flex items-center gap-2">
