...
func TestAffiliateCommissionRespOmitsSensitiveFields(t *testing.T) {
	...
	sensitiveFields := []string{"affiliate_profile_id", "invalid_reason"}
	...
	if !strings.Contains(jsonStr, `"base_amount"`) {
		t.Error("base_amount should appear")
	}
	if !strings.Contains(jsonStr, `"rate_percent"`) {
		t.Error("rate_percent should appear")
	}
}
