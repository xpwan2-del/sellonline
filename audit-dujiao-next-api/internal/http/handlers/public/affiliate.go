...
// CheckAffiliateInviteCode 校验平台代理邀请码是否可用
func (h *Handler) CheckAffiliateInviteCode(c *gin.Context) {
	if h.AffiliateService == nil {
		shared.RespondError(c, response.CodeInternal, "error.user_fetch_failed", nil)
		return
	}
	code := strings.TrimSpace(c.Param("code"))
	usable, row, err := h.AffiliateService.CheckInviteCode(code)
	if err != nil {
	...
	response.Success(c, gin.H{
		...
		"usable": usable,
		"status": status,
		"used":   status == constants.AffiliateInviteCodeStatusUsed,
	})
}
...
func (h *Handler) OpenAffiliate(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrAffiliateSelfOpenNotAllowed):
			shared.RespondError(c, response.CodeBadRequest, "error.forbidden", nil)
...
func (h *Handler) ListAffiliateCommissions(c *gin.Context) {
	...
	rows, total, err := h.AffiliateService.ListUserCommissionRecords(uid, page, pageSize, status)
	if err != nil {
		shared.RespondError(c, response.CodeInternal, "error.user_fetch_failed", err)
		return
	}
	response.SuccessWithPage(c, affiliateCommissionRecordRespList(rows), response.BuildPagination(page, pageSize, total))
}

func affiliateCommissionRecordRespList(rows []service.AffiliateCommissionRecord) []dto.AffiliateCommissionResp {
	result := make([]dto.AffiliateCommissionResp, 0, len(rows))
	for i := range rows {
		item := dto.NewAffiliateCommissionResp(&rows[i].Commission)
		item.BuyerEmail = rows[i].BuyerEmail
		item.BuyerMasked = rows[i].BuyerMasked
