...
func (v *publicProductView) toProductResp() dto.ProductResp {
	...
	resp := dto.ProductResp{
		...
		RealSoldCount:        v.RealSoldCount,
		SoldCountOffset:      v.SoldCountOffset,
...
func (h *Handler) decorateProductStock(product *models.Product, item *publicProductView) {
	...
	if fulfillmentType == constants.FulfillmentTypeUpstream {
		applyPublicSoldCounts(product, item, 0)
	...
	if fulfillmentType == constants.FulfillmentTypeManual {
		...
		for _, sku := range product.SKUs {
			...
			hasActiveSKU = true
			if sku.ManualStockSold > 0 {
				manualSold += int64(sku.ManualStockSold)
			}
		...
		if hasActiveSKU {
			if hasUnlimitedSKU {
				applyPublicSoldCounts(product, item, manualSold)
			...
			if product.ManualStockSold > 0 {
				manualSold = int64(product.ManualStockSold)
			}
			applyPublicSoldCounts(product, item, manualSold)
			item.ManualStockAvailable = constants.ManualStockUnlimited
			item.StockStatus = constants.ProductStockStatusUnlimited
	...
	applyPublicSoldCounts(product, item, autoSold)
...
func applyPublicSoldCounts(product *models.Product, item *publicProductView, realSold int64) {
	if product == nil || item == nil {
		return
	}
	if realSold < 0 {
	...
	offset := product.SoldCountOffset
	if offset < 0 {
		offset = 0
	}
	item.RealSoldCount = realSold
	item.SoldCountOffset = offset
	item.SoldCount = realSold + int64(offset)
}
