...
func (r *GormOrderRepository) GetAnyByOrderNoAndGuest(orderNo, email, password string) (*models.Order, error) {
	...
	query = applyGuestAuthFilter(query, email, password)
	if err := query.Where("order_no = ?", orderNo).First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, err
...
func (r *GormOrderRepository) GetByIDAndGuest(id uint, email, password string) (*models.Order, error) {
	...
	query := r.withChildren(r.db.Preload("Items").Preload("Fulfillment"))
	query = applyGuestAuthFilter(query, email, password)
	if err := query.Where("id = ? AND parent_id IS NULL", id).First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, err
...
// StatsByUser 按状态聚合用户订单数量（不受分页和状态筛选影响）
...
func (r *GormOrderRepository) ListByGuest(email, password string, page, pageSize int) ([]models.Order, int64, error) {
	var total int64
	countQuery := applyGuestAuthFilter(r.db.Model(&models.Order{}), email, password).Where("parent_id IS NULL")
	...
	query := r.withChildren(r.db.Preload("Items").Preload("Fulfillment"))
	query = applyGuestAuthFilter(query, email, password)
	if err := query.Where("parent_id IS NULL").Order("id desc").
		Limit(pageSize).
...
func applyGuestAuthFilter(query *gorm.DB, email, password string) *gorm.DB {
	query = query.Where("user_id = 0 AND guest_password = ?", password)
	if email != "" {
		query = query.Where("guest_email = ?", email)
	}
	return query
}
