...
func TestWalletRepositoryStatsRechargeOrders(t *testing.T) {
	repo, db := setupWalletRepositoryTest(t)
	now := time.Now().UTC().Truncate(time.Second)

	orders := []models.WalletRechargeOrder{
		{
			RechargeNo:      "DJR-STATS-A001",
			UserID:          1,
			PaymentID:       3001,
			ChannelID:       11,
			ProviderType:    constants.PaymentProviderOfficial,
		...
		},
		{
			...
			PayableAmount:   models.NewMoneyFromDecimal(decimal.RequireFromString("30.00")),
			FeeRate:         models.NewMoneyFromDecimal(decimal.Zero),
			FeeAmount:       models.NewMoneyFromDecimal(decimal.Zero),
			Currency:        "CNY",
			Status:          constants.WalletRechargeStatusFailed,
			CreatedAt:       now,
			UpdatedAt:       now,
		},
	}
	if err := db.Create(&orders).Error; err != nil {
		t.Fatalf("create recharge stats orders failed: %v", err)
	}

	stats, err := repo.StatsRechargeOrders(WalletRechargeListFilter{
		UserID: 1,
		Status: constants.WalletRechargeStatusPending,
	})
	if err != nil {
		t.Fatalf("stats recharge orders failed: %v", err)
	}
	if stats[constants.WalletRechargeStatusPending] != 1 {
		t.Fatalf("pending count want 1 got %d", stats[constants.WalletRechargeStatusPending])
	}
	if stats[constants.WalletRechargeStatusSuccess] != 1 {
		t.Fatalf("success count want 1 got %d", stats[constants.WalletRechargeStatusSuccess])
	}
	if stats[constants.WalletRechargeStatusFailed] != 1 {
		t.Fatalf("failed count want 1 got %d", stats[constants.WalletRechargeStatusFailed])
	}

	filteredStats, err := repo.StatsRechargeOrders(WalletRechargeListFilter{
		UserID:     1,
		RechargeNo: "DJR-STATS-A",
	})
	if err != nil {
		t.Fatalf("stats recharge orders by recharge_no failed: %v", err)
	}
	if filteredStats[constants.WalletRechargeStatusPending] != 1 {
		t.Fatalf("filtered pending count want 1 got %d", filteredStats[constants.WalletRechargeStatusPending])
	}
	if filteredStats[constants.WalletRechargeStatusSuccess] != 1 {
		t.Fatalf("filtered success count want 1 got %d", filteredStats[constants.WalletRechargeStatusSuccess])
	}
	if filteredStats[constants.WalletRechargeStatusFailed] != 0 {
		t.Fatalf("filtered failed count want 0 got %d", filteredStats[constants.WalletRechargeStatusFailed])
	}
}
