...
func (s *OrderService) createOrder(input orderCreateParams) (*models.Order, error) {
	...
	err = s.orderRepo.Transaction(func(tx *gorm.DB) error {
		...
		if s.affiliateSvc != nil && input.UserID > 0 && affiliateProfileID != nil {
			if err := s.affiliateSvc.BindCustomerRelationForOrderTx(tx, input.UserID, *affiliateProfileID, affiliateCode, order.ID, now); err != nil {
				return err
			}
		}
