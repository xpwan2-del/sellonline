...
type CreateProductInput struct {
	...
	AffiliateCommissionRate *decimal.Decimal
...
func (s *ProductService) Create(input CreateProductInput) (*models.Product, error) {
	...
	soldCountOffset := normalizeSoldCountOffset(input.SoldCountOffset)
	...
	affiliateCommissionRate, err := normalizeProductAffiliateCommissionRate(input.AffiliateCommissionRate)
	if err != nil {
		return nil, err
	}
	...
	product := models.Product{
		...
		ManualStockLocked:       0,
		ManualStockSold:         0,
		SoldCountOffset:         soldCountOffset,
...
func (s *ProductService) Update(id string, input CreateProductInput) (*models.Product, error) {
	...
	if input.SoldCountOffset != nil {
		product.SoldCountOffset = normalizeSoldCountOffset(input.SoldCountOffset)
	}
	affiliateCommissionRate, err := normalizeProductAffiliateCommissionRate(input.AffiliateCommissionRate)
	if err != nil {
		return nil, err
...
func normalizeSoldCountOffset(value *int) int {
...
func normalizeProductAffiliateCommissionRate(rate *decimal.Decimal) (*models.Money, error) {
	if rate == nil {
		return nil, nil
	}
	normalized := rate.Round(2)
	if normalized.LessThan(decimal.Zero) || normalized.GreaterThan(decimal.NewFromInt(100)) {
		return nil, ErrProductPriceInvalid
	}
	money := models.NewMoneyFromDecimal(normalized)
	return &money, nil
}
