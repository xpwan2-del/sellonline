...
func TestProductServiceCreateAndUpdateSoldCountOffset(t *testing.T) {
	svc, db := newProductServiceForTest(t)

	category := models.Category{Slug: "sold-count-offset", NameJSON: models.JSON{"zh-CN": "sold-count-offset"}}
	if err := db.Create(&category).Error; err != nil {
		t.Fatalf("create category failed: %v", err)
	}
	...
	baseInput := func(slug string) CreateProductInput {
	...
	createInput := baseInput("sold-count-offset-product")
	createInput.SoldCountOffset = intPtr(8)
	created, err := svc.Create(createInput)
	if err != nil {
		t.Fatalf("create product failed: %v", err)
	}
	if created.SoldCountOffset != 8 {
		t.Fatalf("expected sold_count_offset=8 on create, got %d", created.SoldCountOffset)
	}

	idStr := strconv.FormatUint(uint64(created.ID), 10)
	updateInput := baseInput(created.Slug)
	updated, err := svc.Update(idStr, updateInput)
	if err != nil {
		t.Fatalf("update product without sold count offset failed: %v", err)
	}
	if updated.SoldCountOffset != 8 {
		t.Fatalf("expected omitted sold_count_offset to keep 8, got %d", updated.SoldCountOffset)
	}

	negativeInput := baseInput(created.Slug)
	negativeInput.SoldCountOffset = intPtr(-2)
	normalized, err := svc.Update(idStr, negativeInput)
	if err != nil {
		t.Fatalf("update product with negative sold count offset failed: %v", err)
	}
	if normalized.SoldCountOffset != 0 {
		t.Fatalf("expected negative sold_count_offset to normalize to 0, got %d", normalized.SoldCountOffset)
	}
}
