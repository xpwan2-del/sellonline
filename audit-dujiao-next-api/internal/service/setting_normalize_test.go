...
func TestUpdateSiteSettingNormalized(t *testing.T) {
	...
	result, err := svc.Update(constants.SettingKeySiteConfig, map[string]interface{}{
		...
		"contact": map[string]interface{}{
			"qq_group": "  https://qm.qq.com/q/demo  ",
	...
	if contact["qq_group"] != "https://qm.qq.com/q/demo" {
		t.Fatalf("unexpected qq_group: %v", contact["qq_group"])
	}
	if contact["whatsapp"] != "" {
...
func TestUpdateSiteSettingDropsCustomScripts(t *testing.T) {
	repo := newMockSettingRepo()
	svc := NewSettingService(repo)
