...
// SetAffiliateRepository 设置推广返利仓储，用于注册时处理平台代理邀请码和买家归属。
func (s *UserAuthService) SetAffiliateRepository(repo repository.AffiliateRepository) {
	s.affiliateRepo = repo
}
...
// UserRegisterAffiliateInput 注册时的推广返利参数
type UserRegisterAffiliateInput struct {
	AgentInvite         string
...
func (s *UserAuthService) Register(email, password, code string, agreementAccepted bool, emailVerificationEnabled bool) (*models.User, string, time.Time, error) {
	return s.RegisterWithAffiliate(email, password, code, agreementAccepted, emailVerificationEnabled, UserRegisterAffiliateInput{})
}

// RegisterWithAffiliate 用户注册，并处理一级代理准入或买家归属。
func (s *UserAuthService) RegisterWithAffiliate(email, password, code string, agreementAccepted bool, emailVerificationEnabled bool, affiliateInput UserRegisterAffiliateInput) (*models.User, string, time.Time, error) {
	...
	agentInvite := normalizeAffiliateInviteCode(affiliateInput.AgentInvite)
	affiliateCode := normalizeAffiliateCode(affiliateInput.AffiliateCode)
	if agentInvite != "" && affiliateCode != "" {
		return nil, "", time.Time{}, ErrAffiliateRegisterParamsConflict
	}
	if agentInvite != "" {
		invite, err := s.lookupUsableAffiliateInviteCode(agentInvite)
		if err != nil {
			return nil, "", time.Time{}, err
		}
		if invite == nil {
			return nil, "", time.Time{}, ErrAffiliateInviteCodeInvalid
		}
	}
	if affiliateCode != "" {
		profile, err := s.lookupActiveAffiliateProfileByCode(affiliateCode)
		if err != nil {
			return nil, "", time.Time{}, err
		}
		if profile == nil {
	...
	if s.affiliateRepo != nil && (agentInvite != "" || affiliateCode != "") {
		if err := s.affiliateRepo.Transaction(func(tx *gorm.DB) error {
			userRepo := s.userRepo.WithTx(tx)
			affiliateRepo := s.affiliateRepo.WithTx(tx)
			if err := userRepo.Create(user); err != nil {
				return err
			}
			if agentInvite != "" {
				return s.createAffiliateProfileFromInvite(affiliateRepo, user.ID, agentInvite, now)
			}
			if affiliateCode != "" {
				return s.createCustomerRelationFromAffiliateCode(affiliateRepo, user.ID, affiliateCode, constants.AffiliateCustomerRelationSourceRegister, nil, now)
			}
			return nil
		}); err != nil {
			return nil, "", time.Time{}, err
...
func (s *UserAuthService) lookupUsableAffiliateInviteCode(code string) (*models.AffiliateInviteCode, error) {
	if s == nil || s.affiliateRepo == nil {
		return nil, ErrAffiliateInviteCodeInvalid
	}
	invite, err := s.affiliateRepo.GetInviteCodeByCode(code)
	if err != nil {
		return nil, err
	}
	if invite == nil {
	...
	if strings.TrimSpace(invite.Status) != constants.AffiliateInviteCodeStatusActive || invite.UsedByUserID != nil || invite.UsedAt != nil {
		return nil, ErrAffiliateInviteCodeUnavailable
	}
	return invite, nil
}

func (s *UserAuthService) lookupActiveAffiliateProfileByCode(code string) (*models.AffiliateProfile, error) {
	if s == nil || s.affiliateRepo == nil {
		return nil, nil
	}
	profile, err := s.affiliateRepo.GetProfileByCode(code)
	if err != nil {
		return nil, err
	}
...
func (s *UserAuthService) createAffiliateProfileFromInvite(repo repository.AffiliateRepository, userID uint, code string, now time.Time) error {
	if repo == nil || userID == 0 {
		return ErrAffiliateInviteCodeInvalid
	}
	invite, err := repo.GetInviteCodeByCodeForUpdate(code)
	if err != nil {
		return err
	}
	if invite == nil {
	...
	if strings.TrimSpace(invite.Status) != constants.AffiliateInviteCodeStatusActive || invite.UsedByUserID != nil || invite.UsedAt != nil {
		return ErrAffiliateInviteCodeUnavailable
	}
	const maxRetry = 8
	for i := 0; i < maxRetry; i++ {
		affiliateCode, genErr := generateAffiliateCode()
		if genErr != nil {
			return genErr
		}
		profile := &models.AffiliateProfile{
			...
			Source:        constants.AffiliateProfileSourcePlatformInvite,
		...
		rows, err := repo.MarkInviteCodeUsed(invite.ID, userID, now)
		if err != nil {
			return err
		}
		if rows == 0 {
...
func (s *UserAuthService) createCustomerRelationFromAffiliateCode(repo repository.AffiliateRepository, userID uint, code, sourceType string, sourceOrderID *uint, now time.Time) error {
	if repo == nil || userID == 0 {
		return nil
	}
	profile, err := repo.GetProfileByCode(code)
	if err != nil {
		return err
	}
	if profile == nil || strings.TrimSpace(profile.Status) != constants.AffiliateProfileStatusActive {
