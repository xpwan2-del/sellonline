<template>
  <div
    ...
    <!-- Content Area -->
    <div class="p-2.5 md:p-4 relative z-20 flex flex-col flex-1">
      ...
      <h3 class="text-sm md:text-lg font-bold theme-text-primary mb-1 md:mb-2 transition-colors line-clamp-2 md:line-clamp-1 min-h-[2.5rem] md:min-h-0">
      ...
      <p class="theme-text-secondary text-xs md:text-sm mb-3 md:mb-6 line-clamp-2">
        {{ getLocalizedText(product.description) }}
      </p>

      <div v-if="soldCount > 0" class="mb-2 md:mb-3 text-xs font-medium theme-text-muted">
        {{ t('products.soldCount', { count: soldCount }) }}
      </div>
...
const soldCount = computed(() => {
  const value = Number(props.product?.sold_count ?? 0)
