        ...
        <div class="theme-panel rounded-2xl p-6">
          <div class="mx-auto max-w-md space-y-4">
              <button
                ...
                class="inline-flex h-11 w-full items-center justify-center rounded-lg px-5 text-sm font-semibold text-white transition-colors"
                :class="payActionButtonClass">
                {{ payActionLabel }}
              </button>
              <div v-if="openedPayWindow" class="text-center text-xs text-emerald-500">
                {{ payLinkOpenedTip }}
              </div>
              <div v-if="showTelegramPayHint" class="text-center text-xs theme-text-muted">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-6">
          ...
          <div v-if="paymentResult"
            ...
            <div v-if="showPayLink" class="mt-6">
              <button
                type="button"
                @click="handleOpenPayLink"
                class="inline-flex h-10 min-w-40 items-center justify-center rounded-lg px-5 text-sm font-semibold text-white transition-colors"
                :class="payActionButtonClass">
                {{ payActionLabel }}
              </button>
...
const hasGuestAuth = computed(() => Boolean(guestAuth.value.order_password))
...
const paymentBrand = computed(() => {
  const channelType = paymentChannelType.value
  const channelName = String(resultChannelName.value || '').toLowerCase()
  if (channelType === 'alipay' || channelName.includes('支付宝') || channelName.includes('alipay')) return 'alipay'
  if (channelType === 'wechat' || channelType === 'wxpay' || channelName.includes('微信') || channelName.includes('wechat')) return 'wechat'
  return 'default'
})

const payActionLabel = computed(() => {
  return t('payment.confirmPayOrder')
})

const payActionButtonClass = computed(() => {
  if (paymentBrand.value === 'alipay') return 'bg-[#1677ff] hover:bg-[#0f67e8]'
  if (paymentBrand.value === 'wechat') return 'bg-[#22ac38] hover:bg-[#15952b]'
