      ...
      <div v-else class="space-y-6">
        ...
        <div class="theme-panel rounded-2xl p-6">
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <button
              v-if="isPending && payLink"
              type="button"
              @click="handleOpenPayLink"
              class="inline-flex h-11 w-full items-center justify-center rounded-xl bg-[#1677ff] px-5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-[#0f6bdd] md:w-auto"
            >
              {{ t('rechargeOrder.confirmPay') }}
            </button>
          </div>
        </div>
