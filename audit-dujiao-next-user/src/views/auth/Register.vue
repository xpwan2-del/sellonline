    ...
    <div class="relative z-10 w-full max-w-lg">
      ...
      <div class="theme-auth-card">
        ...
        <form class="theme-auth-form" @submit.prevent="handleRegister">
          <div v-if="agentInviteCode" class="rounded-xl border theme-surface-soft px-4 py-3 text-sm theme-text-secondary">
            {{ t('auth.register.agentInviteHint') }}：
            <span class="font-mono theme-text-primary">{{ agentInviteCode }}</span>
          </div>
...
import { clearAffiliateCode, getAffiliateCode, getAffiliateVisitorKey, resolveAffiliateCodeFromRoute } from '../../utils/affiliate'
import { captureAgentInviteFromRoute, checkAgentInviteCode, clearAgentInviteCode, getAgentInviteCode } from '../../utils/affiliateInvite'
...
const performRegister = async () => {
  error.value = ''
  const currentEmail = registrationEmail.value
  const agentInvite = agentInviteCode.value
  const routeAffiliateCode = resolveAffiliateCodeFromRoute(route)
  if (agentInvite && routeAffiliateCode) {
    error.value = t('auth.register.errors.agentInviteAffiliateConflict')
    return
  }
  if (agentInvite) {
    clearAffiliateCode()
  }
  const affiliateCode = agentInvite ? '' : getAffiliateCode()
  if (agentInvite) {
    try {
      const valid = await checkAgentInviteCode(agentInvite)
      if (!valid) {
        error.value = t('auth.register.errors.agentInviteInvalid')
        return
      }
    } catch {
      error.value = t('auth.register.errors.agentInviteInvalid')
  ...
  try {
    await userAuthStore.register({
      ...
      agent_invite: agentInvite || undefined,
      affiliate_code: !agentInvite ? affiliateCode || undefined : undefined,
      affiliate_visitor_key: !agentInvite && affiliateCode ? getAffiliateVisitorKey() || undefined : undefined,
    })
    if (agentInvite) clearAgentInviteCode()
...
onMounted(async () => {
  agentInviteCode.value = captureAgentInviteFromRoute(route) || getAgentInviteCode()
  if (agentInviteCode.value && !resolveAffiliateCodeFromRoute(route)) {
    clearAffiliateCode()
  }
  await appStore.loadConfig(true)
})
