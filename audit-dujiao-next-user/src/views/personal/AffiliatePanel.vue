  ...
  <div class="space-y-6">
    <div class="theme-personal-card">
      ...
      <div v-else class="rounded-xl border border-dashed theme-surface-soft p-5">
        <div class="max-w-2xl">
          <h3 class="text-lg font-bold theme-text-primary">{{ t('personalCenter.affiliate.applyTitle') }}</h3>
          <p class="mt-2 text-sm theme-text-secondary">{{ t('personalCenter.affiliate.applyDescription') }}</p>
        </div>

        <div v-if="agentApplication?.id" class="mt-5 rounded-xl border theme-surface-muted p-4">
          <div class="flex flex-wrap items-center gap-3">
            <span class="text-sm font-semibold theme-text-primary">{{ t('personalCenter.affiliate.applicationStatusTitle') }}</span>
            <span class="theme-badge px-2.5 py-1 text-xs font-semibold" :class="agentApplicationStatusClass(agentApplication.status)">
              {{ agentApplicationStatusLabel(agentApplication.status) }}
            </span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-3 text-sm md:grid-cols-2">
            <div>
              <div class="text-xs theme-text-muted">{{ t('personalCenter.affiliate.applyPhoneLabel') }}</div>
              <div class="mt-1 theme-text-primary">{{ agentApplication.phone || '-' }}</div>
            </div>
            <div>
              <div class="text-xs theme-text-muted">{{ t('personalCenter.affiliate.applicationTime') }}</div>
              <div class="mt-1 theme-text-primary">{{ formatDate(agentApplication.created_at) }}</div>
            </div>
          </div>
          <div v-if="agentApplication.admin_note" class="mt-3 text-sm theme-text-secondary">
            {{ agentApplication.admin_note }}
          </div>
        </div>

        <form v-if="!agentApplication || agentApplication.status === 'rejected'" class="mt-5 grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,260px)_minmax(0,1fr)_auto]" @submit.prevent="handleApplyAgent">
          <div>
            <label class="mb-2 block text-sm font-medium theme-text-secondary">{{ t('personalCenter.affiliate.applyPhoneLabel') }}</label>
            <input
              v-model.trim="agentApplicationForm.phone"
              type="tel"
              class="w-full form-input-lg"
              :placeholder="t('personalCenter.affiliate.applyPhonePlaceholder')"
            />
          </div>
          <div>
            <label class="mb-2 block text-sm font-medium theme-text-secondary">{{ t('personalCenter.affiliate.applyMessageLabel') }}</label>
            <input
              v-model.trim="agentApplicationForm.message"
              type="text"
              class="w-full form-input-lg"
              :placeholder="t('personalCenter.affiliate.applyMessagePlaceholder')"
            />
          </div>
          <div class="flex items-end">
            <button
              ...
              class="inline-flex h-11 w-full items-center justify-center rounded-xl theme-btn-primary px-5 text-sm font-bold transition-colors disabled:cursor-not-allowed disabled:opacity-60 md:w-auto"
            >
              {{ submittingApplication ? t('personalCenter.affiliate.applying') : t('personalCenter.affiliate.applySubmit') }}
            </button>
          </div>
        </form>
      </div>
    ...
    <div v-if="dashboard?.opened" class="theme-personal-card">
      <div class="flex gap-2 overflow-x-auto pb-1">
        <button
          ...
          :key="tab.key"
          type="button"
          class="whitespace-nowrap rounded-xl border px-3 py-2 text-sm font-semibold transition-colors"
          :class="activeTab === tab.key ? 'theme-btn-primary' : 'theme-btn-secondary'"
    ...
    <AffiliateReportSection v-if="dashboard?.opened && activeTab === 'overview'" mode="overview" />
    <AffiliateReportSection v-if="dashboard?.opened && activeTab === 'orders'" mode="orders" />
    <AffiliateCustomerList v-if="dashboard?.opened && activeTab === 'customers'" />

    <div v-if="dashboard?.opened && activeTab === 'withdraw'" class="theme-personal-card">
    ...
    <div v-if="dashboard?.opened && activeTab === 'commissions'" class="theme-personal-card">
        ...
        <table class="min-w-full divide-y divide-gray-200 text-left text-sm dark:divide-white/10">
          <thead class="bg-gray-50/80 text-xs uppercase tracking-wide text-gray-500 dark:bg-white/5 dark:text-gray-400">
            <tr>
              ...
              <th class="px-4 py-3 font-semibold">{{ t('personalCenter.affiliate.table.buyerEmail') }}</th>
              <th class="px-4 py-3 font-semibold">{{ t('personalCenter.affiliate.table.rate') }}</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-white/10">
            <tr v-for="item in commissions" :key="item.id">
              <td class="px-4 py-3 font-mono text-xs theme-text-primary">{{ item.order?.order_no || '-' }}</td>
              <td class="px-4 py-3 text-xs theme-text-secondary">{{ item.buyer_email || item.buyer_masked || '-' }}</td>
              <td class="px-4 py-3 font-mono text-xs theme-text-primary">{{ item.rate_percent || '0.00' }}%</td>
              <td class="px-4 py-3 text-xs theme-text-primary">
                <div class="font-mono">{{ item.commission_amount }}</div>
                <details v-if="commissionItems(item).length" class="mt-2">
                  <summary class="cursor-pointer theme-text-accent">{{ t('personalCenter.affiliate.table.details') }}</summary>
                  <div class="mt-2 min-w-[460px] rounded-lg border border-gray-200/70 p-2 dark:border-white/10">
                    <div class="grid grid-cols-[1.4fr_0.8fr_0.5fr_0.8fr_0.7fr_0.8fr] gap-2 border-b border-gray-200/70 pb-1 text-[11px] theme-text-muted dark:border-white/10">
                      <span>{{ t('personalCenter.affiliate.items.product') }}</span>
                      <span>{{ t('personalCenter.affiliate.items.sku') }}</span>
                      <span>{{ t('personalCenter.affiliate.items.quantity') }}</span>
                      <span>{{ t('personalCenter.affiliate.items.baseAmount') }}</span>
                      <span>{{ t('personalCenter.affiliate.items.rate') }}</span>
                      <span>{{ t('personalCenter.affiliate.items.commission') }}</span>
                    </div>
                    <div
                      v-for="row in commissionItems(item)"
                      :key="row.id || `${row.order_item_id}-${row.product_id}`"
                      class="grid grid-cols-[1.4fr_0.8fr_0.5fr_0.8fr_0.7fr_0.8fr] gap-2 py-1 text-[11px]"
                    >
                      <span class="break-words">{{ commissionItemTitle(row) }}</span>
                      <span class="break-words theme-text-muted">{{ commissionItemSKU(row) }}</span>
    ...
    <div v-if="dashboard?.opened && activeTab === 'withdraws'" class="theme-personal-card">
      <div class="mb-4 flex items-center justify-between">
...
import { affiliateAPI, type AffiliateAgentApplicationData, type AffiliateCommissionData, type AffiliateDashboardData, type AffiliateWithdrawData } from '../../api'
import {
...
import AffiliateCustomerList from './affiliate/AffiliateCustomerList.vue'
import AffiliateReportSection from './affiliate/AffiliateReportSection.vue'
...
const agentApplication = ref<AffiliateAgentApplicationData | null>(null)
const panelAlert = ref<PageAlert | null>(null)
const activeTab = ref<'overview' | 'orders' | 'customers' | 'commissions' | 'withdraw' | 'withdraws'>('overview')

const commissions = ref<AffiliateCommissionData[]>([])
const withdraws = ref<AffiliateWithdrawData[]>([])
const affiliateTabs = [
  { key: 'overview', label: t('personalCenter.affiliate.tabs.overview') },
  { key: 'orders', label: t('personalCenter.affiliate.tabs.orders') },
  { key: 'customers', label: t('personalCenter.affiliate.tabs.customers') },
  { key: 'commissions', label: t('personalCenter.affiliate.tabs.commissions') },
  { key: 'withdraw', label: t('personalCenter.affiliate.tabs.withdraw') },
  { key: 'withdraws', label: t('personalCenter.affiliate.tabs.withdraws') },
] as const

const commissionItems = (item: AffiliateCommissionData) => Array.isArray(item.commission_items) ? item.commission_items : []

const commissionItemTitle = (item: NonNullable<AffiliateCommissionData['commission_items']>[number]) => {
...
const commissionItemSKU = (item: NonNullable<AffiliateCommissionData['commission_items']>[number]) => {
...
const agentApplicationForm = reactive({
  phone: '',
  message: '',
})
...
const loadAgentApplication = async () => {
  try {
    const response = await affiliateAPI.agentApplication()
    const data = response.data.data as AffiliateAgentApplicationData | undefined
    agentApplication.value = data?.id ? data : null
  } catch {
...
const handleApplyAgent = async () => {
  panelAlert.value = null
  if (!agentApplicationForm.phone.trim()) {
    panelAlert.value = {
      level: 'warning',
      message: t('personalCenter.affiliate.errors.applyPhoneRequired'),
    }
    return
  }
  ...
  try {
    const response = await affiliateAPI.applyAgent({
      phone: agentApplicationForm.phone.trim(),
      message: agentApplicationForm.message.trim() || undefined,
    })
    const data = response.data.data as AffiliateAgentApplicationData | undefined
    agentApplication.value = data?.id ? data : null
    agentApplicationForm.phone = ''
    agentApplicationForm.message = ''
    panelAlert.value = {
      level: 'success',
      message: t('personalCenter.affiliate.applySuccess'),
    }
  } catch (err: any) {
    panelAlert.value = {
      level: 'error',
      message: err?.message || t('personalCenter.affiliate.errors.applyFailed'),
    }
  } finally {
...
const agentApplicationStatusLabel = (status?: string) => {
  if (status === 'pending') return t('personalCenter.affiliate.applicationStatus.pending')
  if (status === 'contacted') return t('personalCenter.affiliate.applicationStatus.contacted')
  if (status === 'rejected') return t('personalCenter.affiliate.applicationStatus.rejected')
  return status || '-'
}

const agentApplicationStatusClass = (status?: string) => {
  if (status === 'pending') return 'theme-badge-warning'
  if (status === 'contacted') return 'theme-badge-info'
  if (status === 'rejected') return 'theme-badge-neutral'
  return 'theme-badge-neutral'
}
